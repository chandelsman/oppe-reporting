...
consult_iop <- read_excel("./input/iop_q1_20.xls")
...
# Intraop summary table
consult_iop %>% 
  filter(!is.na(Correlation), cl_grp == "UC Health South") %>% 
  group_by(`IntraOp Type`) %>% 
  summarize(No = sum(Correlation == "NO"),
            NwC = sum(Correlation == "NO with Comments"),
            Yes = sum(Correlation == "YES"),
            YwC = sum(Correlation == "YES with Comments")) %>% 
  # select(`Primary Pathologist`, NO, Nwc, YES, YwC) %>% 
  gt(rowname_col = "IntraOp Type", auto_align = TRUE) %>% 
  # tab_row_group(group = `IntraOp Path`) %>% 
  tab_header (title = "Intraoperative Consulations",
              subtitle = "") %>% 
  summary_rows(
    groups = NULL,
    columns = vars(No, NwC, Yes, YwC),
