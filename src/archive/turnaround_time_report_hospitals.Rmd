...
params:
  cl_grp: Ivinson Memorial Hospital
---
...
```{r table TAT for client group drop 88311 and 88341, message = FALSE, warning = FALSE}
...
<br><br>
<div align="center"> <h3> Statistics by Case Type: `r params$cl_grp` </div>
...
```{r table TAT by case type no 88311 or 88341, message = FALSE, warning = FALSE}
...
ta_time %>%
  ...
  gt(rowname_col = "type", auto_align = TRUE) %>%
  tab_header (title = "Without CPT Codes 88331 and 88341",
              subtitle = "") %>%
  fmt_number(columns = vars(Cases),
             decimals = 0,
             use_seps = TRUE) %>%
...
consult_iop <- consult_iop %>%
  filter(Correlation != "Select", !is.na(`IntraOp Path`)) %>% 
  mutate(
    `Sequence Group` = str_replace(`Sequence Group`, "\\*SURG", "SURG"),
    `Sequence Group` = str_replace(`Sequence Group`, "OP SURG", "SURG OP"),
    `Sequence Group` = str_replace(`Sequence Group`, "OP NGYN", "NGYN OP"),
