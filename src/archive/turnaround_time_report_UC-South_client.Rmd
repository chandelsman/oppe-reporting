...
# This script calculates turnaround time for clients associated with the 
# UC Health South client group client groups as specified
# in the YAML header. Summaries are provided for each 
...
ta_time_q1 <- 
  bind_rows(ta_time_01, ta_time_02, ta_time_03) # combine 
...
ta_time <- ta_time_q1 %>% 
  mutate(
    ...
    client = case_when(
      ...
      `SEQUENCE GROUP` == "NGYN (MHC)" ~ "Memorial Central",
      `SEQUENCE GROUP` == "NGYN (MHCC DC)" ~ "Memorial Hospital of Converse County",
      `SEQUENCE GROUP` == "NGYN (MHN)" ~ "Memorial North",
      ...
      `SEQUENCE GROUP` == "NGYN (WOODLAND)" ~ "Woodland Park",
      ...
      `SEQUENCE GROUP` == "SURG (GRANDVIEW)" ~ "Grandview",
      ...
      `SEQUENCE GROUP` == "SURG (MHC)" ~ "Memorial Central",
      `SEQUENCE GROUP` == "SURG (MHCC DC)" ~ "Memorial Hospital of Converse County",
      `SEQUENCE GROUP` == "SURG (MHN)" ~ "Memorial North",
      ...
      `SEQUENCE GROUP` == "SURG (WOODLAND)" ~ "Woodland Park",
...
```{r calculate_turnaround_time,message = FALSE, warning = FALSE}
...
<div align="center"> <h3> Turnaround Time Summary: `r params$client` </div>
...
```{r table TAT for client group drop 88311 and 88341, warning = FALSE}

ta_time %>% 
  filter(client == params$client, !(str_detect(CPTS, "88311 | 88341"))) %>% 
  summarize(
    Cases = n(),
    Average = mean(span),
    `Under 48 hrs` = sum(span <= 48) / n()
  ...
  gt(auto_align = TRUE) %>% 
  tab_header (title = "2020 Quarter 1 Without CPT Codes 88311 and 88341",
              subtitle = "") %>% 
  fmt_number(columns = vars(Cases),
             decimals = 0,
             use_seps = TRUE) %>% 
...
<br><br>
<div align="center"> <h3> Statistics by Case Type: `r params$client` </div>


```{r table TAT by case type, message = FALSE, warning = FALSE}
...
```{r table TAT by facility no 88311 or 88341, message = FALSE, warning = FALSE}
...
ta_time %>% 
  ...
  gt(rowname_col = "type", auto_align = TRUE) %>% 
  tab_header (title = "Without CPT Codes 88311 and 88341",
    subtitle = "") %>% 
  fmt_number(columns = vars(Cases),
             decimals = 0,
             use_seps = TRUE) %>% 
...
<br>
<div align="center"> <h3> Statistics by Pathologist: `r params$client` </div>


```{r table TAT by pathologist, message = FALSE, warning = FALSE}

ta_time %>% 
  filter(client == params$client, PATHOLOGIST != "Long, Daniel") %>%
...
<P style="page-break-before: always">
<br>
<div align="center"> <h3> Internal Consultations: `r params$client` </div>


```{r internal consultations, message = TRUE, warning = FALSE}
...
<br><br>
<div align="center"> <h3> External Consultations: `r params$client` </div>


```{r external consultations, warning = FALSE, message = FALSE}

# Load external consult data ----------------------------------------------

consult_ext <- read_excel("./input/ext_q1_20.xls")
...
```{r table summary of external consultations, message = FALSE, warning = FALSE}

consult_ext %>% 
  filter(!is.na(Correlation), client == params$client) %>% 
...
<P style="page-break-before: always">
<br>
<div align="center"> <h3> Intraoperative Consultations: `r params$client` </div>


```{r intraoperative consultations, message = FALSE, warning = FALSE}

# Load intraoperative consult data ------------------------------------------------------
...
```{r table summary of intraoperative consultations, message = FALSE, warning = FALSE}
...
<br><br>
<div align="center"> <h3> Intraoperative Consultations by Pathologist: `r params$client` </div>

```{r consult_intraoperative by pathologist, warning = FALSE}


# Intraop table by pathologist
consult_iop %>% 
  filter(!is.na(Correlation), client == params$client,
         `IntraOp Path` != "Long, Daniel") %>% 
