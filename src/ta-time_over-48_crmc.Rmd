...
params:
  ...
  #   - 4
  cl_grp:
    # - Kimball County Hospital ### Not using???
    # - Memorial Hospital of Converse County ### Client inactive after 2021 Q3
...
```{r load-functions, include = FALSE, message = FALSE, warning = FALSE}
# Import functions --------------------------------------------------------
...
# Remove missing data  
ta_time <- ta_time %>% filter(!is.na(span))
```
...
## **`r lubridate::date(params$dt_start)` to `r lubridate::date(params$dt_end)`** 
...
ta_time %>%
  summarize(
    n = n(),
    Average = mean(span),
    `n > 48 hrs` = sum(span >= 48),
    `Percent > 48 hrs` = sum(span >= 48) / n()
  ) %>%
  gt(auto_align = TRUE) %>%
  tab_header(title = "",
              subtitle = "") %>%
...
## Turnaround Time > 48 hours by Case Type 
...
## Turnaround Time > 48 hours by Pathologist 
...
## Cases with Turnaround Time > 48 hour

  </div>
</div>

```{r cases_over_48hrs, message = FALSE, warning = FALSE}
ta_time %>%
  filter(span >= 48) %>%
  select(`RESULT ID`, PATHOLOGIST) %>%
  arrange(PATHOLOGIST, `RESULT ID`) %>% 
  gt(auto_align = TRUE) %>%
  tab_header(title = "",
             subtitle = "") %>%
  cols_label(`RESULT ID` = "Result ID",
...
```{r tat_table_pathologist_filtered, message = FALSE, warning = FALSE}
